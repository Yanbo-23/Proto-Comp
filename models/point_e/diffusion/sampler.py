...
import numpy as np
from models.point_e.util.point_cloud import PointCloud
...
class PointCloudSampler:
    ...
    def uniform_sample_t(self, batch_size):
        ts = np.random.choice(
            np.arange(1, self.diffusions[0].betas.shape[0]), batch_size
        )
        return torch.tensor(ts, device=self.device)

    def loss_texts(self, partial, reals, cond, batch_size):
        ...
        output = self.diffusions[0].training_losses(
            ...
            reals.permute(0, 2, 1),
            t,
            model_kwargs=dict(texts=cond),
        )

        return output["loss"].mean()

    def loss_images(self, reals, cond, batch_size):
        t = self.uniform_sample_t(batch_size)
        output = self.diffusions[0].training_losses(
            self.models[0], reals.permute(0, 2, 1), t, model_kwargs=dict(images=cond)
        )

        return output["loss"]

    def sample_batch_progressive(
        self, partial: torch.Tensor, batch_size: int, model_kwargs: Dict[str, Any]
        ...
        for (
            ...
            try:
                if hasattr(model.module, "cached_model_kwargs"):
                    stage_model_kwargs = model.module.cached_model_kwargs(
