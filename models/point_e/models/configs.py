...
def model_from_config(
    config: Dict[str, Any], proto_config: Dict[str, Any], device: torch.device
) -> nn.Module:
    if proto_config == None:
        config = config.copy()
    else:
        config = config.copy()
        proto_config = proto_config.copy()
    ...
    if name == "PointDiffusionTransformer":
        ...
        return CrossAttentionPointCloudSDFModel(
            device=device, dtype=torch.float32, **config
        )
