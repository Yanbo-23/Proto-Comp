...
def jitter_points(pc, std=0.01, clip=0.05):
    ...
    for i in range(bsize):
        jittered_data = (
            pc.new(pc.size(1), 3).normal_(mean=0.0, std=std).clamp_(-clip, clip)
        )
...
def build_lambda_sche(opti, config, last_epoch=-1):
    if config.get("decay_step") is not None:
        ...
        scheduler = torch.optim.lr_scheduler.LambdaLR(
            opti, lr_lbmd, last_epoch=last_epoch
        )
    else:
...
def seprate_point_cloud_cpu(
    xyz, num_points, crop, fixed_points=None, padding_zeros=False
):
    """
    seprate point cloud: usage : using to generate the incomplete point cloud with a setted number.
    """
    ...
    for points in xyz:
        ...
        if isinstance(crop, list):
            ...
            INPUT.append(random_sample(input_data, 2048))
            CROP.append(random_sample(crop_data, 2048))
        else:
            INPUT.append(input_data)
            CROP.append(crop_data)
